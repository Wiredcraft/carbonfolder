...
function styleGenerate(selector, parent, object, ret){
  var element, onSelector, signal;

  if (ret === undefined)
    ret = '';

  ret = ret.concat(selector, '{');
  
  for (var property in object[selector]) {
    var value = object[selector][property];
    
    if (typeof(value) === "string") {
      ret = ret.concat(property, ':', value, ';');
    } else {
      ret += '}';
      styleGenerate(property, element[i], object[selector]);
    }
  }
}
...
Jss.apply = function(obj, element) {
  if (typeof element === 'undefined')
    element = document;
  
  ready(function() {
    for (var selector in obj) {
      styleApply(selector, element, obj);
    }
  });  
};
