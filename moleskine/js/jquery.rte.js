...
lwRTE.prototype.enable_design_mode = function() {
  ...
  var doc = "<html><head>" + base + css + "</head><body " + style + " style='padding:5px; font-family : \"Helvetica Neue\",Helvetica,Arial,sans-serif'>" + content + "</body></html>";
...
lwRTE.prototype.disable_markdown = function(submit) {
  var self = this;
  ...
  var doc = "<html><head>" + base + css + "</head><body " + style + " style='padding:5px'>" + htmlContent + "</body></html>";
...
lwRTE.prototype.enable_markdown = function(submit) {
  var self = this;
  ...
  var markdown = self.reMarked.render($('body', self.iframe_doc).html());

  $(self.textarea).val(markdown);

  $(self.iframe).before(self.textarea);  
  if(!self.toolbars.md)
    self.toolbars.md	= self.create_toolbar(self.controls.md);

  if(submit != true) {
    $(self.iframe_doc).remove(); //fix 'permission denied' bug in IE7 (jquery cache)
    $(self.iframe).remove();
    self.iframe = self.iframe_doc = null;
    self.activate_toolbar(self.textarea, self.toolbars.md);
  }
...
lwRTE.prototype.disable_design_mode = function(submit) {
  ...
  console.log(self.controls.html);
